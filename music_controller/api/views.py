...
class LeaveRoom(APIView):
    ...
    def post(self,request,format="None"):
        if not self.request.session.exists(self.request.session.session_key):
            return Response({'Bad Request': 'Session already not active'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'Bad Request': 'Room Code doesnt exist'}, status=status.HTTP_400_BAD_REQUEST)
...
class UpdateRoom(APIView):
    ...
    def patch(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            print(f"Received data - guest_can_pause: {guest_can_pause}, votes_to_skip: {votes_to_skip}, code: {code}")
